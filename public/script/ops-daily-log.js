$(function(){
    ...
    $('body').on('click', '#apply-filter', function(){
        ...
        var url = `daily-log/list?keyword=true&limit=${limit || 100}${date_begin}${date_end}${shift}${pelanggan_id}${equipment_id}${karyawan_id}`
        if(!pelanggan_id){
            // body.find('filtermodal').modal('hide')
            swal('Opps', 'Harus memilih pelanggan utk menampilkan data....', 'error')
        }else{
            $.ajax({
                async: true,
                url: url,
