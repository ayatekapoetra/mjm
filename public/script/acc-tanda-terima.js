$(function(){
    ...
    $('body').on('click', 'button.bt-clear-item', function(e){
        ...
        elmTr.find('select[name="coa_kredit"]').val(null).trigger('change')
        elmTr.find('select[name="gudang_id"]').val(null).trigger('change')
        elmTr.find('div#optionSource, div#optionFaktur').html('')
    })

    $('body').on('change', 'select[name="coa_kredit"]', function(e){
        var elm = $(this)
        var values = elm.val()
        var workdir = body.find('input#workdir').val()
        ...
        if(values){
            $.ajax({
                async: true,
                // url: '/ajax/coa/check-transaksi?bisnis_id='+workdir+'&selected='+values,
    ...
    $('body').on('change', 'select[name="barang_id"]', function(e){
        ...
        if(values){
            $.ajax({
                ...
                contentType: false,
                success: function(result){
                    elm.parents('tr.item-rows').find('select[name="coa_kredit"]').val(result.coa_in).trigger('change')
    ...
    $('body').on('change', 'select[name="barang_id"]', function(e){
        ...
        var elmCoa = $(this).parents('tr.item-rows').find('select[name="coa_id"]')
        var elmCoaID = $(this).parents('tr.item-rows').find('select[name="coa_id"] > option[kode="'+kode+'"]').attr('value')
        if(idBarang){
            elmCoa.val(elmCoaID)
