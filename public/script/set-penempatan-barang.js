$(function(){
    ...
    $('body').on('click', '#apply-filter', function(){
        ...
        var url = `penempatan-barang/list?keyword=true&limit=${limit}${kode}${serial}${num_part}${nama}${satuan}`
        $.ajax({
            async: true,
            url: url,
            method: 'GET',
        ...
        swal({
          ...
          function(){
              $.ajax({
                  async: true,
                  headers: {'x-csrf-token': $('[name=_csrf]').val()},
                  url: 'penempatan-barang/'+id+'/destroy',
                  method: 'DELETE',
                  dataType: 'json',
                  processData: false,
                  mimeType: "multipart/form-data",
    ...
    $('body').on('submit', 'form#form-update', function(e){
        ...
        $.ajax({
            async: true,
            headers: {'x-csrf-token': $('[name=_csrf]').val()},
            url: 'penempatan-barang/'+id+'/update',
            method: 'POST',
            data: data,
            dataType: 'json',
            processData: false,
