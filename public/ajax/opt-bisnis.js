$(function(){
    ...
    $('select.selectBisnis').select2()

    $('body select.selectBisnis').each(function(){
        var elm = $(this)
        var values = $(this).data('values') || elm.val()
        // console.log(values, workdir);
        $.ajax({
            async: true,
            url: '/ajax/options/bisnis?selected='+values,
            method: 'GET',
            dataType: 'json',
            processData: false,
            ...
            success: function(result){
                ...
                if(result.length > 0){
                    ...
                    elm.html(result.map( v => '<option value="'+v.id+'" '+v.selected+'>'+v.initial+' -> '+v.name+'</option>'))
