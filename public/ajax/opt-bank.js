$(function(){
    console.log('ajax/opt-bank');

    var body = $('body')

    $('select.selectBank').select2()
    ...
    $('select[name="bank_id"]').each(function(){
        ...
        $.ajax({
            async: true,
            url: '/ajax/options/bank?selected='+values,
            method: 'GET',
            data: {
    ...
    $('select[name="bank_src"], select[name="bank_target"]').each(function(){
        var elm = $(this)
        var values = $(this).data('values') || elm.val()
        $.ajax({
            async: true,
            url: '/ajax/options/bank',
            method: 'GET',
            data: {
                selected: values || null
            },
