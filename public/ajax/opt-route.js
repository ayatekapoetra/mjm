$(function(){
    console.log('ajax/opt-route');

    var body = $('body')

    $('select.selectRoute').select2()

    $('body select.selectRoute').each(function(){
        var elm = $(this)
        var values = $(this).data('values') || elm.val()
        $.ajax({
            async: true,
            url: '/ajax/options/routex?selected='+values,
            method: 'GET',
            dataType: 'json',
            processData: false,
            ...
            success: function(result){
                ...
                if(result.length > 0){
                    ...
                    elm.html(result.map( v => '<option value="'+v.id+'" '+v.selected+'>'+v.menu.name+' --> '+v.name+'</option>'))
